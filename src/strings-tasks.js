...
function removeFirstOccurrences(str, value) {
  ...
  if (idx !== -1) {
    return str.slice(0, idx) + str.slice(idx + value.length);
  }
  return str;
}
...
function formatTime(minutes, seconds) {
  const min =
    minutes < 10 ? minutes.toString().padStart(2, '0') : seconds.toString();
  const sec =
...
function findLongestWord(sentence) {
  const lengthWordsArr = sentence.split(' ');
  const sortArr = lengthWordsArr.sort((a, b) => b.length - a.length);
  return sortArr[0];
}
...
function extractNameFromTemplate(value) {
  const newStr = value.replace(/[!?,]/g, '').split(' ');
  newStr.shift();
  return newStr.join(' ');
}
...
function encodeToRot13(str) {
  const arrStr = [...str];
  const codeArr = arrStr.map((letter) => {
    if (/\W/gi.test(letter)) {
      return letter;
    }
    const codeLetter = letter.charCodeAt();
    if (letter === letter.toUpperCase()) {
      return codeLetter + 13 <= 90
        ? String.fromCharCode(codeLetter + 13)
        : String.fromCharCode(64 + (13 - (90 - codeLetter)));
    }
    return codeLetter + 13 <= 122
      ? String.fromCharCode(codeLetter + 13)
      : String.fromCharCode(96 + (13 - (122 - codeLetter)));
  });
